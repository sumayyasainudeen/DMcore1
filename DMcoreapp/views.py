...
def he_work_asign(request,pk):
    ...
    tls=user_registration.objects.filter(department='Team Lead')
    tlids = [tl.id for tl in tls]
    works = Work.objects.filter(tl_id__in=tlids,client_name=client)
    return render(request,'head/he_work_asign.html',{'client':client,'tls':tls,"usr":usr,"works":works})

def he_view_work_asign_client(request,tlid):
    ids=request.session['userid']
    usr = user_registration.objects.get(id=ids)
    wrk_as = work_asign.objects.filter(tl_id=tlid)
    dist_clients = wrk_as.values('client_name').distinct()
    print(dist_clients)
    clients=client_information.objects.filter(id__in=dist_clients)
    return render(request,'head/he_view_work_asign_client.html',{"usr":usr,"clients":clients,"tlid":tlid})

def he_view_work_asign_exe(request,cid,tlid):
    ids=request.session['userid']
    usr = user_registration.objects.get(id=ids)
...
def he_work_add(request,id):
    ...
    if request.method == 'POST':
        ...
        for i in array:
            tl=user_registration.objects.get(department="Team Lead",fullname=i)
            w=Work(task=task,description=des,start_date=sdate,end_date=edate,file_attached=file,cl_name=client.bs_name,client_name=client,tl_id=tl.id)
            w.save()
            ...
            c=work_asign(work_id=w.id,tl_id=tl.id,client_name_id=client.id)
            c.save()
...
def tl_profile(request):
    ids=request.session['userid']
    usr = user_registration.objects.get(id=ids)
    return render(request, 'team_lead/tl_profile.html',{"usr":usr})
...
def tl_view_work_asign_exe(request,id):
    ...
    client=client_information.objects.get(id=id)
    work=work_asign.objects.filter(client_name=client.id,exe_name__in=exe_ids)
...
def tl_daily_task(request):
    ...
    exe=user_registration.objects.filter(department='Digital Marketing Executive',tl_id=ids)
    exe_ids = [exe.id for exe in exe]
    work=daily_work.objects.filter(date=today,user_id__in=exe_ids)
    master=Work.objects.all()
    sub_work=daily_work_sub.objects.all()
    dl_sub=daily_work_sub.objects.all() 
    dl_off=daily_off_sub.objects.all()
...
def tl_workprogress_home(request):
    ids=request.session['userid']
    usr = user_registration.objects.get(id=ids)
    return render(request,'team_lead/tl_workprogress_home.html',{"usr":usr,})
...
def tl_workprogress_executive(request):
    ...
    usr = user_registration.objects.get(id=ids)
    exe=user_registration.objects.filter(department='Digital Marketing Executive',tl_id=ids)
    return render(request,'team_lead/tl_workprogress_executive.html',{'exe':exe,"usr":usr,})

def tl_workprogress_executive_det(request,eid):
    ids=request.session['userid']
    usr = user_registration.objects.get(id=ids)
    # exe=user_registration.objects.filter(department='Digital Marketing Executive',tl_id=ids)
    # exe_ids = [exe.id for exe in exe]
...
def tl_workprogress_monthly(request):
    ids=request.session['userid']
    usr = user_registration.objects.get(id=ids)
    exe=user_registration.objects.filter(department='Digital Marketing Executive',tl_id=ids)
    exe_ids = [exe.id for exe in exe]
...
def tl_all_leads_exe(request):
    ids=request.session['userid']
    usr = user_registration.objects.get(id=ids)
    tls=user_registration.objects.filter(department="Digital Marketing Executive",tl_id=ids)
    context={
        "usr":usr,
        "tls":tls
...
def tl_executive_home(request):
    ids=request.session['userid']
    usr = user_registration.objects.get(id=ids)
    executives = user_registration.objects.filter(department='Digital Marketing Executive',tl_id=ids)
...
def tl_waste_data_home(request):
    ids=request.session['userid']
    usr = user_registration.objects.get(id=ids)
    return render(request, 'team_lead/tl_waste_data_home.html', {'usr':usr})

def tl_waste_data_exe(request):
    ids=request.session['userid'] 
    usr = user_registration.objects.get(id=ids)
    exe=user_registration.objects.filter(department='Digital Marketing Executive',tl_id=ids)
    exe_names = [exe.fullname for exe in exe]
    leads_by_date = daily_leeds_exists.objects.filter(executive__in=exe_names).values('date').annotate(total_leads=Count('id')).order_by('-date')
    ...
    return render(request, 'team_lead/tl_waste_data_exe.html', context)

def tl_waste_data_det_exe(request,date=None):
    if request.session.has_key('userid'):
        ids = request.session['userid']
    else:
        return redirect('/')
    ...
    return render(request,'team_lead/tl_waste_data_det_exe.html', context)
...
def tl_waste_data(request):
    ...
    status_values = ["Rejected","Not interested","Will contact if interested","Already contacted","Already joined in Altos",
                     "Already joined for another training","Not interested for payment","Recently join the job",
                     "Already done another internship recently","looking for direct job","payment is not affordable"]
    leads_by_date = All_leads.objects.filter(status__in=status_values,executive_id__in=exe_ids).values('assign_dt').annotate(total_leads=Count('id')).order_by('-assign_dt')
    ...
    return render(request, 'team_lead/tl_waste_data.html', context)

def tl_waste_data_det(request,date=None):
    if request.session.has_key('userid'):
        ids = request.session['userid']
    else:
        return redirect('/')
    ...
    return render(request,'team_lead/tl_waste_data_det.html', context)

def tl_leave_home(request):
    if 'userid' in request.session:
        ...
        return render(request, 'team_lead/tl_leave_home.html',{'usr':usr})
    else:
        return redirect('/')

def tl_leave_aply(request):
    if 'userid' in request.session:
        ids=request.session['userid']
        usr = user_registration.objects.get(id=ids)
        return render(request, 'team_lead/tl_leave_apply.html',{'usr':usr})
    else:
        return redirect('/')
...
def tl_leave_form(request):
    if 'userid' in request.session:
        ...
        if request.method == "POST":
            ...
            leaves.save()
        return  redirect('tl_all_leave')  
    else:
        return redirect('/')

def tl_all_leave(request):
    if 'userid' in request.session:
        if 'userid' in request.session:
            ids=request.session['userid']
            usr = user_registration.objects.get(id=ids)
        ...
        mem = user_registration.objects.filter(id=ids)
        var = leave.objects.filter(user_id=ids)
        return render(request, 'team_lead/tl_all_leave.html',{'var': var,'usr':usr})
    else:
        return redirect('/')

def tl_exe_smopost(request):
    ids=request.session['userid']
    usr = user_registration.objects.get(id=ids)
    exe=user_registration.objects.filter(department="Digital Marketing Executive",tl_id=ids)
    ...
    return render(request, 'team_lead/tl_exe_smopost.html',context) 

def tl_sv_smopost(request,id):
    ids=request.session['userid']
    usr = user_registration.objects.get(id=ids)
    post = smo_post.objects.filter(executive=id)
    addi_post=addi_smo_post.objects.all()
    ...
    return render(request, 'team_lead/tl_sv_smopost.html',context)


#-----------------------------------------------------------------sumayya--dm head-----------------------------------------------------------
def he_assign_tl(request):
    if 'userid' in request.session:
        if 'userid' in request.session:
            ids=request.session['userid']
            usr = user_registration.objects.get(id=ids)
        ...
        for e in exes:
            if e.tl_id == 0:
                ...
                e.tl = user_registration.objects.get(id = e.tl_id).fullname
        return render(request, 'head/he_assign_tl.html',{'usr':usr,'exes':exes,'tls':tls})
    else:
        return redirect('/')

def he_assign_tl1(request):
    if 'userid' in request.session:
            ids=request.session['userid']
            usr = user_registration.objects.get(id=ids)
    else:
        return redirect('/')
    if request.method == "POST":
        tl_id = request.POST['tl']
        eid = request.POST['eid']
        ex = user_registration.objects.get(id=eid)
        ex.tl_id = tl_id
        ex.save()
        return HttpResponseRedirect(request.META.get('HTTP_REFERER'))

def he_view_work_asign_tl(request):
    ids=request.session['userid']
    usr = user_registration.objects.get(id=ids)
    tls=user_registration.objects.filter(department="Team Lead")
    ...
    return render(request, 'head/he_view_work_asign_tl.html',context) 

# def he_view_work_asign_ex(request):
